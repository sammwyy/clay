...
#define CLAY_COMPACT_SYSTEM_PROMPT                                                                                   \
    "You are compacting a coding-assistant conversation so it can continue with less context. Read the "            \
    "transcript below and write a concise summary that preserves: what the user asked for, key decisions "          \
    "made, files touched and why, the current state of the task, and anything still pending. Tool call "            \
    "results were stripped to short previews to save space - if an important detail seems to be missing "           \
    "because of that, say so explicitly instead of guessing. Plain prose, no meta-commentary about being a "        \
    "summarizer."

static char *collapse_for_compact(const char *text) {
    if (!text) return strdup("");
    size_t len = strlen(text);
    if (len <= CLAY_COMPACT_TOOL_PREVIEW) return strdup(text);
    ClayStr out;
    clay_str_init(&out);
...
char *clay_commands_build_compact_transcript(ClayJson *conversation) {
    ...
    for (size_t i = 1; i < clay_json_array_count(conversation); i++) {
        ...
        if (strcmp(role, "user") == 0) {
            ...
            if (content && *content) clay_str_printf(&out, "Assistant: %s\n\n", content);
            ...
            for (size_t j = 0; j < clay_json_array_count(tool_calls); j++) {
                ...
                const char *name = clay_json_string_value(clay_json_object_get(function, "name"));
                const char *arguments = clay_json_string_value(clay_json_object_get(function, "arguments"));
...
static void accumulate_token(const char *text, void *userdata) {
    if (text && *text) clay_str_push(userdata, text);
}
...
void clay_cmd_compact(const char *args, void *user_data) {
    ...
    if (!commands->selected_provider || !commands->selected_model) {
        clay_sayc(CLAY_RED, "Select a provider and model with /model before compacting.");
        return;
    }
    ...
    char *transcript = clay_commands_build_compact_transcript(commands->conversation);
    if (!*transcript) {
        free(transcript);
        clay_sayc(CLAY_GRAY, "Nothing to compact yet.");
        return;
    ...
    clay_json_array_push(messages, clay_openai_message("system", CLAY_COMPACT_SYSTEM_PROMPT));
    ...
    clay_app_task_success(commands->app, task, "%zu messages -> summary", before_count);

    ClayJson *new_conversation = clay_json_array();
    clay_json_array_push(new_conversation, clay_json_clone(clay_json_array_get(commands->conversation, 0)));
    ClayStr note;
    clay_str_init(&note);
    clay_str_printf(&note, "Summary of the conversation so far (compacted with /compact):\n%s", summary.data);
    clay_json_array_push(new_conversation, clay_openai_message("system", note.data));
    clay_str_free(&note);
    clay_str_free(&summary);
    ...
    clay_sayc(CLAY_GREEN, "Compacted. The conversation now starts from a summary.");
}
