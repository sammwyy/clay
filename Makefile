...
ASK_USER_TEST_SRC    := tests/test_ask_user.c
...
SUBAGENT_TEST_SRC    := tests/test_subagent.c
SUBAGENT_TEST_OBJ    := $(patsubst tests/%.c,$(BUILD_DIR)/tests/%.o,$(SUBAGENT_TEST_SRC))
SUBAGENT_TEST_TARGET := $(BIN_DIR)/test_subagent
...
TEST_OBJ_ALL := $(CLI_TEST_OBJ) $(COMMAND_TEST_OBJ) $(CHAT_TEST_OBJ) \
	...
	$(TODOWRITE_TEST_OBJ) $(ASK_USER_TEST_OBJ) $(TASKS_TEST_OBJ) $(MODES_TEST_OBJ) $(SUBAGENT_TEST_OBJ) $(PROCESS_TEST_OBJ) $(MCP_TEST_OBJ) \
...
UNIT_TEST_TARGETS := $(CLI_TEST_TARGET) $(COMMAND_TEST_TARGET) $(CHAT_TEST_TARGET) \
	...
	$(TODOWRITE_TEST_TARGET) $(ASK_USER_TEST_TARGET) $(TASKS_TEST_TARGET) $(MODES_TEST_TARGET) $(SUBAGENT_TEST_TARGET) $(PROCESS_TEST_TARGET) $(MCP_TEST_TARGET) \
...
test-ask-user: $(ASK_USER_TEST_TARGET)
	@./$<
...
test-subagent: $(SUBAGENT_TEST_TARGET)
	@./$<

$(SUBAGENT_TEST_TARGET): $(LIB_OBJ) $(SUBAGENT_TEST_OBJ) | $(BIN_DIR)
	$(CC) $(LIB_OBJ) $(SUBAGENT_TEST_OBJ) -o $@ $(LDFLAGS)
