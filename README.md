...
| **Four modes, one shortcut** | Shift+Tab cycles **Sandbox (Ask)** → **Sandbox (Auto)** → **Unleashed (Ask)** → **Unleashed (Auto)**. The first half picks where commands run, the second whether each tool call waits for your approval; the live status shows both, Ask in pink and Auto in red. |
