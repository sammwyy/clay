...
| **Delegation, when it earns it** | For a job with several real parts the model writes a `todowrite` plan, then hands whole steps to `subagent`: a fresh agent with the same tools and none of the conversation, briefed by a prompt the caller writes. It works alone and returns a summary that feeds the next step. Every run is kept whole under `~/.clay/chats/<chat>/subagents/<run>.json`. Simple tasks skip all of it. |
